(function() {
  var CSSLint, LintCache, Parser, async, crypto, defaultLessOptions, findLessMapping, findPropertyLineNumber, getPropertyName, path, _ref;
  ...
  module.exports = function(grunt) {
    var isFileError, lintCss, originalPositionFor, parseLess, stripPath, writeToFormatters;
    stripPath = require('grunt-lib-contrib').init(grunt).stripPath;
    parseLess = function(file, less, options, callback) {
      var configLessOptions, error, lessOptions, parser, _ref1;
    ...
    lintCss = function(css, options, callback) {
      var cssLintOptions, enabled, externalOptions, id, result, rules, _ref1, _ref2;
      if (!css) {
        callback(null, []);
        return;
      }
    ...
    return grunt.registerMultiTask('lesslint', 'Validate LESS files with CSS Lint', function() {
      ...
      queue = async.queue(function(file, callback) {
        var cache, hash, less, processFile;
        ...
        processFile = function() {
          return parseLess(file, less, options, function(error, less, css) {
            if (error != null) {
              errorCount++;
            ...
            return lintCss(css, options, function(error, result) {
              var cssLine, errorPrefix, filePath, fullRuleMessage, line, lineNumber, message, messages, rule, ruleMessage, ruleMessages, _i, _len, _ref2, _ref3;
              if (result == null) {
                result = {};
              }
              ...
              if (messages.length > 0) {
                ...
                return cache.addCached(hash, function(error) {
                  if (error != null) {
        ...
        if (options.cache) {
          ...
          return cache.hasCached(hash, function(isCached) {
            if (isCached) {
